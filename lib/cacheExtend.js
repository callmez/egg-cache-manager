 ...
 * @param {number} ttl - cache expire in milliseconds
 * @return {Promise<*>}
 */
Policy.prototype.take = async function({ key, force, create, dependency, format, ttl } = {}) {
  let value = force === true ? null : await this.get(key);
  value = await this._resolveDependency(value);

  if (value === null && create) value = await create();

  const cacheValue = value !== null && dependency ? await this._evaluateDependency(value, dependency) : value;
  if (cacheValue !== null) await this.set(key, cacheValue, ttl);

  if (value !== null && format) value = await format(value);
